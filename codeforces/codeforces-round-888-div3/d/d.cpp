...
auto solve() {
     ...
     **/
    std::cin >> n;
    vl a = Reader::vector<ll>(n - 1);
    vi b(n + 2);
    ll invalid = -1;
    a.insert(a.begin(), 0);
    int idx;
    for (int i: range(1, sz(a))) {
        ll diff = a[i] - a[i - 1];
        if (diff > n || b[diff] == 1) {
            invalid = diff;
            idx = i;
    ...
    // trace(invalid _ unused);
    if (sz(unused) != 2) {
        return false;
    }
    return unused[0] + unused[1] == invalid;
    // a.insert(a.begin() + idx, a[idx - 1] + unused[0]);
    // vi t;
    // for (int i: range(1, sz(a))) {
    //     t.push_back(a[i] - a[i - 1]);
    // }
    // sort(all(t));
    // return t == range(1, n + 1);
}
