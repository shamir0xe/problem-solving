...
auto solve() {
    ...
    auto on_bit = [&](int number, int bit) { return number >> bit & 1; };
    vi res;
    int best_idx = 1;
    for (int i: range(1, n)) {
        int lcp = 0;
        vi cur;
        while (lcp < k) {
            while (lcp < k && on_bit(a[order[i]], k - lcp - 1) ==
                                  on_bit(a[order[i - 1]], k - lcp - 1)) {
                lcp++;
            }
            cur.push_back(lcp);
            lcp++;
        ...
        // trace(i _ lcp);
        smax(res, cur, [&] { best_idx = i; });
    }
    int x = (a[order[best_idx - 1]] ^ ((1 << k) - 1)) &
            (a[order[best_idx]] ^ ((1 << k) - 1));
    // trace(((a[order[best_idx - 1]] ^ x) & (a[order[best_idx]] ^ x)));
    return vi{order[best_idx - 1] + 1, order[best_idx] + 1, x};
}
