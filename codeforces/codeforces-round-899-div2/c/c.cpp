...
ll fn(int idx, int can) {
    if (idx >= n) return 0;
    ll &res = dp[idx][can];
    if (res != -1)
        return res;
    res = 0;
    smax(res, fn(idx + 1, can ^ 1));
    if (can) {
        // odd
        smax(res, arr[idx] + fn(idx + 1, can));
        smax(res, arr[idx] + fn(idx + 2, can));
    } else {
        // even
        smax(res, fn(idx + 1, can));
        smax(res, fn(idx + 2, can));
    }
    return res;
}
auto solve() {
