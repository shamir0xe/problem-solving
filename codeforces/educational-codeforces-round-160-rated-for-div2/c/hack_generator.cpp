...
int main() {
    ...
    std::uniform_int_distribution<> randint(0, 10);
    std::cout << test_case << "\n";
    std::array<int, 4> ins{0, 0, 0, 0};
    int total = test_case / 2;
    for (int i: range(test_case - total)) {
        std::cout << 1 << ' ' << ins[dist(gen)] << "\n";
    }
    std::array<int, 4> outs{100000000, 10000, 10000, 0};
    for (int i: range(total)) {
        std::cout << 2 << ' ' << std::max(0, outs[dist(gen)] - randint(gen)) << std::endl;
    }
}
