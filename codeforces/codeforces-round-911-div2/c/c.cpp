...
auto solve() {
    ...
    for (int i: range(n)) {
        ...
        std::cin >> le >> ri;
        if (le != 0) {
            adj[i].emplace_back(le - 1, 0);
        }
        if (ri != 0) {
            adj[i].emplace_back(ri - 1, 1);
        }
    }
    vi order;
    root(0, order);
    ...
    for (int i: range(n)) {
        ...
        if (sz(adj[u]) == 0) {
            ...
            for (auto [v, lr]: adj[u]) {
                int label = labels[u] == 'U' ? -1 : labels[u] == 'L' ? 0 : 1;
                int change = label != lr ? 1 : 0;
                smin(res, change + dp[v]);
            }
        }
    }
    return dp[0];
