...
/**
 * problem D (4/8)
 * codeforces, codeforces-round-905-div3
 * author: @shamir0xe
...
int read_input() {
    ...
    for (int i: range(n)) {
        ...
        queries.emplace_back(ch, left, right);
    }
    return 0;
}
...
int e0() { return 0; }
int mapping(F f, int x) { return x + f.b; }
...
auto solve() {
    ...
    std::map<int, int> mp;
    std::set<int> numbers;
    for (auto &[ch, left, right]: queries) {
        numbers.insert(left);
        numbers.insert(right);
    }
    for (auto &t: numbers) {
        mp[t] = sz(mp);
    }
    // define sg here
    int n = sz(mp);
    atcoder::lazy_segtree<int, max, e0, F, mapping, composition, id> sg(sz(mp));
    std::vector<std::string> ans;
    // trace(n);
    int cur = 0;
    for (auto &[ch, left, right]: queries) {
        // trace(ch _ mp[left] _ mp[right]);
        if (ch == '+') {
            sg.apply(mp[left], mp[right] + 1, F{1});
            cur++;
        } else {
            sg.apply(mp[left], mp[right] + 1, F{-1});
            cur--;
        }
        int maxi = sg.prod(0, n);
        // trace(maxi _ cur);
        if (maxi != cur) {
            ans.push_back("YES");
        } else {
