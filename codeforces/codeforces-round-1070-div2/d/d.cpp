...
/**
 * problem D (4/6)
 * codeforces, codeforces-round-1070-div2
 * author: @shamir0xe
...
auto solve() {
  ...
  int n, m;
  std::cin >> n >> m;
  vl a = Reader::vector<ll>(n);
  std::vector<vi> adj(n, vi());
  range(i, m) {
    int u, v;
    std::cin >> u >> v;
    u--, v--;
  ...
  for (auto v: order) {
    for (auto u: adj[v]) {
      ll cur = a[v] - a[u];
      if (cur > 0 && mp[u].find(cur) != mp[u].end()) {
        ans = add(ans, mp[u][cur]);
        total[v] = add(total[v], mp[u][cur]);
      }
      if (mp[v].find(a[u]) == mp[v].end()) {
        mp[v][a[u]] = 0;
      }
      mp[v][a[u]] = add(mp[v][a[u]], total[u]);
      total[v]++;
      ans = add(ans, total[u]);
      ans = add(ans, 1);
    }
