 ...
 **/
void flip_query(int x) { std::cout << "? 2 " << x << std::endl; }
void first_query(int x) { std::cout << "? 1 1 " << x << std::endl; }
...
auto solve() {
    ...
    vi ans(n, -1);
    first_query(1);
    ans[0] = Reader::primitive<int>();
    if (abs(ans[0]) == 1) {
        // root is 1
        for (int i: range(1, n)) {
            first_query(i + 1);
            ans[i] = Reader::primitive<int>() - ans[0];
        }
    } else {
        // root is anonymous
        flip_query(1);
        first_query(1);
        int alter = Reader::primitive<int>();
        int original_p1 = (ans[0] - alter) / 2;
