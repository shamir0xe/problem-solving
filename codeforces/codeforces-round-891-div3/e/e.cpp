...
auto solve() {
    ...
    vl le(n), ri(n);
    for (int i: range(1, n)) {
        le[i] = le[i - 1] + (y[i] - y[i - 1]) * 1ll * i;
    }
    for (int i: rrange(n - 1)) {
        ri[i] = ri[i + 1] + (y[i + 1] - y[i]) * 1ll * (n - i - 1);
    }
    for (int i: range(n)) {
        int j = std::lower_bound(all(y), x[i]) - y.begin();
        // trace(j _ le[j] _ ri[j]);
        ans.push_back(le[j] + ri[j] + n);
    }
    return ans;
}
