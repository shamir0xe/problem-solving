...
/**
 * problem E (5/7)
 * codeforces, codeforces-round-895-div3
 * author: @shamir0xe
...
int read_input() {
    ...
    while (q--) {
        ...
        if (t == 1) {
            int left, right;
            std::cin >> left >> right;
            queries.emplace_back(1, left - 1, right - 1);
        } else {
            int g;
            std::cin >> g;
            queries.emplace_back(2, g, -1);
        }
    }
    return 0;
}
...
S e0() { return S{0, 0}; }
S operation(S a, S b) { return S{a.zeros ^ b.zeros, a.ones ^ b.ones};}
struct F {
    int s;
};
F id() { return F{0}; }
S mapping(F f, S x) { return f.s == 1 ? S{x.ones, x.zeros} : x; }
F composition(F f1, F f2) { return F{f1.s ^ f2.s}; }
...
auto solve() {
    ...
    std::vector<S> array;
    for (int i: range(n)) {
        array.push_back(str[i] == '0' ? S{arr[i], 0} : S{0, arr[i]});
    }
    atcoder::lazy_segtree<S, operation, e0, F, mapping, composition, id> sg(
        array);
    vi ans;
    // trace(n);
    for (int i: range(sz(queries))) {
        ...
        if (t == 1) {
            ...
            sg.apply(left, right + 1, F{1});
        } else {
            int g = std::get<1>(queries[i]);
            S res = sg.all_prod();
            ans.push_back(g == 0 ? res.zeros : res.ones);
        }
    }
    return ans;
}
