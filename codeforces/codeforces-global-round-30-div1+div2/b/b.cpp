...
auto solve() {
    ...
    range(i, n) {
        if (va[i] % 2 == 0) {
            if (t == -1) {
                ...
                std::cout << t << ' ' << va[i] << std::endl;
                return 0;
            }
        }
    }
    vi sample;
    range(i, std::min(33, n)) {
        if (t != va[i]) {
            sample.push_back(va[i]);
            if (t >= 0) {
                int r = -1;
                if (t < va[i]) {
                    r = va[i] % t;
                ...
                if (r % 2 == 0) {
                    std::cout << std::min(t, va[i]) << ' ' << std::max(t, va[i])
                              << std::endl;
                    return 0;
                }
            }
    ...
    range(i, n) {
        fr(j, i + 1, n) {
            int r = sample[j] % sample[i];
            if (r % 2 == 0) {
                std::cout << sample[i] << ' ' << sample[j] << std::endl;
                return 0;
            }
        }
    }
