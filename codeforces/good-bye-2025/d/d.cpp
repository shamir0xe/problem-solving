...
vi solve(vi &b) {
  ...
  while (loop) {
    ...
    range(i, n / 2) {
      ...
      if (h[b[i + tt]] > 0) {
        loop = true;
        ans.push_back(vi{b[n - 1 - i], b[i + tt]});
        h[b[i + tt]] -= a[b[n - 1 - i]];
        h[b[n - 1 - i]] -= a[b[i + tt]];
      }
    }
...
auto solve() {
  ...
  h.assign(all(a));
  vi order;
  range(i, n) { order.push_back(i); }
  std::sort(all(order), [&](int i, int j) { return a[i] > a[j]; });
  trace(order);
  if (m > n / 2) {
    return vvi{};
  }
  ...
  vi b;
  if (2 * m < n) {
    b.assign(order.begin() + 2 * m, order.end());
    b = solve(b);
  }

  if (sz(b) && m > 0) {
    ans.push_back(vi{b[0], order[2 * m - 1]});
  }
  range(i, m) { ans.push_back(vi{order[i], order[2 * m - 1 - i]}); }
...
int second_main() {
  ...
  if (n == 0) {
    ...
    std::cout << n << "\n";
    range(i, n) { std::cout << ans[i][0] + 1 << ' ' << ans[i][1] + 1 << "\n"; }
  }
