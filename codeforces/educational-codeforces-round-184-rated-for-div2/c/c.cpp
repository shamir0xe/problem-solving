...
/**
 * problem C (3/6)
 * codeforces, educational-codeforces-round-184-rated-for-div2
 * author: @shamir0xe
...
auto solve() {
    ...
    int n;
    std::cin >> n;
    vi ai = Reader::vector<int>(n);
    vi aai = vi(all(ai));
    range(i, n) { ai[i] -= 2 * (i + 1); }
    std::vector<ll> psum(n);
    psum[0] = ai[0];
    std::set<std::pair<ll, int>> cur;
    cur.insert({0, -1});
    cur.insert({-psum[0], 0});
