...
/**
 * problem D (4/6)
 * codeforces, educational-codeforces-round-164-rated-for-div2
 * author: @shamir0xe
...
auto solve() {
    ...
    for (int i: range(n)) {
        ...
        for (int j: range(maxn)) {
            if (j < a) {
                ans = add(ans, mul(a, ways[side][j]));
            } else {
                ans = add(ans, mul(ceil((a + j) / 2.), ways[side][j]));
            }
        }
        ways[side ^ 1] = ways[side];
        for (int j: range(maxn)) {
            if (j + a >= maxn) {
                break;
            }
            ways[side ^ 1][j + a] = add(ways[side ^ 1][j + a], ways[side][j]);
        }
        side ^= 1;
    }
    return ans;
