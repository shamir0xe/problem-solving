...
auto solve() {
    ...
    for (int i: range(n)) {
        pq.emplace(trailing_zeros(arr[i]), arr[i], i);
    }
    for (int t: queries) {
        while (true) {
            auto [zeros, number, idx] = pq.top();
            if (t > zeros) {
                break;
            }
            pq.pop();
            pq.emplace(t - 1, number | 1 << (t - 1), idx);
        }
    }
    while (!pq.empty()) {
        auto [zeros, number, idx] = pq.top();
