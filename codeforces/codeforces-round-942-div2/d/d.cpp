...
/**
 * problem D (4/6)
 * codeforces, codeforces-round-942-div2
 * author: @shamir0xe
...
int calc(int a, int b) {
    ...
    int ans = 0;
    trace(b _ factors[b]);
    for (int mask: range(1, 1 << sz(factors[b]))) {
        trace(mask);
        int cur = 1;
        int sign = 1;
        for (int i: range(sz(factors[b]))) {
            if (mask >> i & 1) {
                cur *= factors[b][i];
                sign ^= 1;
            }
...
auto solve() {
    ...
    for (int t: range(1, m + 1)) {
        // t: [1, m]
        ll g_max = (t + std::sqrt(t * 1. * t + 4. * n)) / 2;
        smin(g_max, ll(1. * m / t));
        ll g_min = 1;
        smax(g_min, ll((t - std::sqrt(t * 1. * t + 4. * n)) / 2));
        smax(g_min, ll(t + 1));
        if (g_max < g_min) {
            continue;
        }
        trace(g_min _ g_max _ t);
        ans += calc(g_max, t) - calc(g_min - 1, t);
    }
    return ans;
}
