...
auto solve() {
    ...
    ll a, b;
    std::cin >> a >> b;
    std::pair<ll, ll> king, queen;
    std::cin >> king.first >> king.second;
    std::cin >> queen.first >> queen.second;
    auto get_possibles = [&](std::pair<ll, ll> king) {
        std::set<std::pair<ll, ll>> possibles;
        possibles.insert({a + king.X, king.Y + b});
        possibles.insert({a + king.X, king.Y - b});
        possibles.insert({-a + king.X, king.Y - b});
        possibles.insert({-a + king.X, king.Y + b});
    ...
    auto queen_poss = get_possibles(queen);
    int cnt = 0;
    for (auto node: get_possibles(king)) {
        if (queen_poss.count(node) == 1) {
            cnt ++;
        }
    }
    return cnt;
