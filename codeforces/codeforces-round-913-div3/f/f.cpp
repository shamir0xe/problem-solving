...
std::tuple<int, bool> scenario(vi a, int num) {
    switch (num) {
    case 0: {
        for (int i: range(1, n)) {
            if (a[i] < a[i - 1]) {
    ...
    case 1: {
        ...
        while (a[idx] == min && a[(idx - 1 + n) % n] == min) {
            idx = (idx - 1 + n) % n;
        }
        int j = (idx + 2) % n;
        bool can = true;
        ...
        return {std::min(idx + 2, n - idx), can};
        break;
    }
    case 2:
    case 3: {
        ...
        return {std::min(n - idx, idx + 2), can};
        break;
    }
    }
    return {1e9, false};
