...
pii expand(pii t, int k) { return {t.first - k, t.second + k}; }

std::tuple<pii, bool> merge(pii seg1, pii seg2) {
    if (seg1.X > seg2.Y || seg1.Y < seg2.X) {
        return {seg1, false};
    }
    return {pii{std::max(seg1.X, seg2.X), std::min(seg1.Y, seg2.Y)}, true};
}
...
bool possible(int k) {
    ...
    for (int i: range(n)) {
        base = expand(base, k);
        auto [seg, can] = merge(base, segments[i]);
        if (!can) {
            return false;
        }
        base = seg;
