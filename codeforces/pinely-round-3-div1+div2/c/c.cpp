...
/**
 * problem C (3/9)
 * codeforces, pinely-round-3-div1+div2
 * author: @shamir0xe
...
auto solve() {
     ...
     **/
    std::cin >> n;
    vi le = Reader::vector<int>(n);
    vi ri = Reader::vector<int>(n);
    vi cee = Reader::vector<int>(n);
    std::vector<pii> events;
    for (int i: range(n)) {
        events.emplace_back(le[i], 1);
        events.emplace_back(ri[i], 2);
    }
    sort(all(events));
    vi intervals;
    vi stack;
    for (auto event: events) {
        if (event.second == 1) {
            stack.push_back(event.first);
        } else {
            intervals.push_back(event.first - stack.back());
            stack.pop_back();
        }
    }
    sort(all(intervals));
