...
/**
 * problem C (3/6)
 * codeforces, codeforces-round-910-div2
 * author: @shamir0xe
...
auto solve() {
    ...
    int k;
    std::cin >> n >> m >> k;
    std::vector<std::vector<std::vector<char>>> ans(
        2, std::vector<std::vector<char>>(n, std::vector<char>(m)));
    int path = n - 1 + m - 1;
    if (path <= k && ((k - path) % 4 == 0 || (k - path) % 4 == 2)) {
        for (int i: range(n)) {
            for (int j: range(m)) {
                ans[0][i][j] = (i + j) % 2 ? 'R' : 'B';
                ans[1][i][j] = (i + j) % 2 ? 'B' : 'R';
        ...
        for (int i: range(m)) {
            ans[1][0][i] = (m - 1) % 2 == 1 ? 'R' : 'B';
        }
        for (int i: range(n - 1)) {
            ans[1][i][m - 1] = (m - 1 + i) % 2 ? 'R' : 'B';
        }
        for (int i: range(m - 1)) {
            ans[0][1][i] = (m - 1) % 2 == 1 ? 'B' : 'R';
        }
    } else {
        return std::vector<std::vector<std::vector<char>>>();
    }
    return ans;
}
