...
auto solve() {
    ...
    for (int i: range(n)) {
        intervals.push_back({std::min(a[i], b[i]), std::max(a[i], b[i])});
    }
    int i0 = -1;
    for (int i: range(n)) {
        if (i0 == -1 || intervals[i].second < intervals[i0].second) {
            i0 = i;
        }
    }
    int j0 = -1;
    for (int j: range(n)) {
        if (j0 == -1 || intervals[j].first >= intervals[j0].first) {
            j0 = j;
        }
    }
    if (intervals[i0].second < intervals[j0].first) {
        std::swap(b[i0], b[j0]);
    }
    ll ans = 0;
    for (int i: range(n)) {
