...
/**
 * problem D (4/6)
 * codeforces, codeforces-round-915-div2
 * author: @shamir0xe
...
auto solve() {
    ...
    int idx = std::min_element(all(a)) - a.begin();
    vi b;
    for (int i: range(n)) {
    ...
    ll ans = n;
    while (stack.back() != 0) {
        int before = le[stack.back()];
        ans += 1ll * b[stack.back()] * (stack.back() - before);
    ...
    while (stack.back() > 0) {
        int idx = stack.back();
        stack.pop_back();
        cur -= 1ll * b[idx] * (idx - stack.back());
        if (stack.back() == idx - 1) continue;
        vi temp{idx - 1};
        while (stack.back() != temp.back()) {
            temp.push_back(le[temp.back()]);
        }
        temp.pop_back();
        reverse(all(temp));
        for (int i: temp) {
            cur += 1ll * (i - stack.back()) * b[i];
            stack.push_back(i);
        }
        smax(ans, cur);
    }
