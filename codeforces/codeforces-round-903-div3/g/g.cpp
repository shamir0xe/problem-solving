...
template <class S, auto op, auto e, auto collect> class Node {
  public:
    ...
    inline bool contains(int idx) {
        return interval.X <= idx && idx <= interval.Y;
    }
    inline bool leaf() { return interval.X == interval.Y; }
    void relax() {
...
auto solve() {
    ...
    SegTree<int, sum_, e_0, max_> pals(vi(2 * n + 2, 0));
    for (int i: range(n)) {
        vi indices;
    ...
    for (auto &v: queries) {
        if (v[0] == 1) {
            ...
            int le = v[1] - 1;
            int ri = v[2] - 1;
            int temp = pals.get_interval(2 * le + 1, 2 * ri - 1);
