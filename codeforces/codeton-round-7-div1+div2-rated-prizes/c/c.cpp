...
std::tuple<bool, vi> solve() {
    ...
    vi order_a = range(n);
    vi order_b = range(n);
    sort(all(order_a), [&](int i, int j) { return a[i] < a[j]; });
    sort(all(order_b), [&](int i, int j) { return b[i] < b[j]; });
    vi ans(n);
    for (int i: range(x)) {
        ans[order_a[n - i - 1]] = b[order_b[x - i - 1]];
    }
    for (int i: range(x, n)) {
        ans[order_a[n - i - 1]] = b[order_b[n - i - 1 + x]];
    }
    // trace(ans);
    int test = 0;
    for (int i: range(n)) {
...
int second_main() {
    ...
    if (std::get<0>(ans) == false) {
        ...
        std::cout << "YES\n";
        for (int i: range(sz(std::get<1>(ans)))) {
            std::cout << std::get<1>(ans)[i]
                      << " \n"[i == sz(std::get<1>(ans)) - 1];
        }
    }
    return 0;
}
