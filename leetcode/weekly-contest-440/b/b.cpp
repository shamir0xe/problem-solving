...
class Solution {
  public:
    std::vector<long long> findMaxSum(std::vector<int>& nums1, std::vector<int>& nums2, int k) {
        ...
        std::vector<std::pair<pii, int> > pairs;
        for (int i: range(n)) {
            pairs.push_back({{nums1[i], nums2[i]}, i});
        }
        sort(all(pairs));
        std::multiset<int> st;
        ll sum = 0;
        for (int i: range(n)) {
            if (i - 1 >= 0 && pairs[i - 1].X.X == pairs[i].X.X) res[pairs[i].Y] = res[pairs[i - 1].Y];
            else res[pairs[i].Y] = sum;
            st.insert(pairs[i].X.Y);
            sum += pairs[i].X.Y;
            if (sz(st) > k) {
                sum -= *st.begin();
                st.erase(st.begin());
