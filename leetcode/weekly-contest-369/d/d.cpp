...
class Solution {
  public:
    int maximumPoints(std::vector<std::vector<int>> &edges,
        ...
        vvi nvalue(n, vi(maxt + 1, 0));
        for (int i: range(n)) {
            nvalue[i][0] = coins[i];
            for (int t: range(1, maxt + 1)) {
                nvalue[i][t] = nvalue[i][t - 1] / 2;
            }
        }
        trace("hello");
        vi order;
        std::function<void(int, int)> dfs = [&](int u, int papa) {
            for (int v: adj[u]) {
                if (v == papa) {
                    continue;
                }
        ...
        // reverse(all(order));
        trace(order);
        vvi dp(n + 1, vi(maxt, 0));
        for (int i: range(n)) {
            int u = order[i];
            for (int t: rrange(maxt - 1)) {
                int &res = dp[u][t];

                int sum0 = nvalue[u][t] - k;
                for (int v: adj[u]) {
                    sum0 += dp[v][t];
                }
                smax(res, sum0);

                int sum1 = nvalue[u][t + 1];
                for (int v: adj[u]) {
                    sum1 += dp[v][t + 1];
                }
                smax(res, sum1);
