...
class Solution {
  public:
    int numberOfGoodPartitions(std::vector<int> &nums) {
        ...
        for (int i: range(m)) {
            if (sz(stack) == 0 || pairs[i].first > stack.back().second) {
                stack.push_back(pairs[i]);
            } else {
                pii &back = stack.back();
                back.second = std::max(back.second, pairs[i].second);
            }
        }
        return fast_pow(2, sz(pairs), 1e9 + 7);
    }
};
