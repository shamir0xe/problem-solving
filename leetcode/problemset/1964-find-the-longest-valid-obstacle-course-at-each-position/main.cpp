...
/**
 * problem 1964-find-the-longest-valid-obstacle-course-at-each-position (1/1)
 * leetcode, problemset
 * author: @shamir0xe
...
class Solution {
  public:
    ...
    longestObstacleCourseAtEachPosition(std::vector<int> &obstacles) {
        ...
        for (int i: range(n)) {
            int &res = arr[i] = 1;
            smax(res, 1 + sg.prod(0, mp[obstacles[i]] + 1));
            sg.set(mp[obstacles[i]], res);
        }
        return arr;
    }
};
