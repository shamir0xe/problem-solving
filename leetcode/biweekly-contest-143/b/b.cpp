...
public:
  int maxFrequency(std::vector<int> &nums, int k, int numOperations) {
    ...
    for (auto &t: events) {
      if (t.Y == -1) {
        depth += 1;
        if (depth == maxi) intervals.push_back({t.X, -1});
      } else {
        if (depth == maxi) intervals.back().Y = t.X;
        depth -= 1;
      } 
    }
    ...
    for (auto &interval: intervals) {
      candids.insert(interval.X);
      candids.insert(interval.Y);
    }

    int bef = -1;
    int ans = 0;
    for (int t: candids) {
      int i = std::lower_bound(all(nums), t) - nums.begin();
      int j = std::lower_bound(all(nums), t + 1) - nums.begin();
      int length = j - i;
      int a = std::upper_bound(all(nums), t - k - 1) - nums.begin();
      a = i - a;
      int b = std::lower_bound(all(nums), t + k + 1) - nums.begin();
      b = b - j;

      smax(ans, length + std::min(a + b, numOperations));
    }
    return ans;
  }

  std::string assert_yellow(std::string name) { return "Yellow " + name + "!"; }
};
