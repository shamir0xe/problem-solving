...
public:
    int minimumMoves(vector<vector<int>>& grid) {
        atcoder::mcf_graph<int, int> graph(20);
        int source = 2 * 9 + 1;
        int sink = 2 * 9;
        vi sinks, sources;
        for (int i: range(3)) {
            for (int j: range(3)) {
                int node = i * 3 + j;
                if (grid[i][j] == 0) sinks.push_back(node);
                else if (grid[i][j] > 1) sources.push_back(node);
            }
        }
        for (int u: sources) {
            for (int v: sinks) {
                ...
                i = u / 3; j = u % 3;
                ii = v / 3; jj = v % 3;
                graph.add_edge(u, v, 1, abs(i - ii) + abs(j - jj));
            }
        }
        for (int u: sources) {
            int i, j;
            i = u / 3; j = u % 3;
            graph.add_edge(source, u, grid[i][j] - 1, 0);
        }
        for (int v: sinks) {
            graph.add_edge(v, sink, 1, 0);
        }
